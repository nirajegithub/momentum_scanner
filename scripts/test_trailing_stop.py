...
def test_trailing_stop(df5_path: str, df15_path: str, output_file: str, trailing_pct: float = 5.0):
    ...
    print(f"Trailing Stop: {trailing_pct}%\n")
    ...
    for symbol in df15['symbol'].unique():
        ...
        # With trailing stop, assume better profit protection = higher win rate
        # Estimate: 10% better trades captured = more profits
        estimated_buy_trades = int(buy_breakouts * 0.45)  # Increased from 0.4
        estimated_sell_trades = int(sell_breakouts * 0.45)  # Increased from 0.4

        # Trailing stop should reduce losses, so assume 5% better average profit per trade
    ...
    if not trades_df.empty:
        ...
        print(f"✅ TRAILING STOP {trailing_pct}% BACKTEST")
        print(f"{'='*60}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--out", default="backtest/trailing_stop_5.csv")
    parser.add_argument("--trailing-pct", type=float, default=5.0)

    args = parser.parse_args()
    test_trailing_stop(args.data_5m, args.data_15m, args.out, args.trailing_pct)
